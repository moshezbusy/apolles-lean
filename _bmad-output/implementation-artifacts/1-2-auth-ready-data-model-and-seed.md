...
- [x] Task 2: Create and apply initial migration (AC: #3)
  ...
  - [x] Run Prisma Migrate successfully (`prisma migrate dev` in development and `prisma migrate deploy` workflow in production-like environments), rather than relying only on manual SQL application.
...
- [x] [AI-Review][High] Refresh story File List so it only claims files with current review evidence; current list does not match the active git diff scope. [_bmad-output/implementation-artifacts/1-2-auth-ready-data-model-and-seed.md:246]
- [x] [AI-Review][High] Complete native Prisma Migrate execution with a working direct database path so AC #3 is actually satisfied. [_bmad-output/implementation-artifacts/1-2-auth-ready-data-model-and-seed.md:29]
- [x] [AI-Review][Medium] Update seed runtime to prefer `DIRECT_URL` when available so `pnpm db:seed` can avoid pooled Supabase connections the same way Prisma CLI config does. [apolles/prisma/seed.ts:39]
- [x] [AI-Review][Medium] Remove predictable fallback seed passwords or gate them behind explicit local-dev-only behavior to avoid shipping known credentials. [apolles/prisma/seed.ts:144]
- [x] [AI-Review][Medium] Make seed user upserts preserve existing role/activation state, or scope seed writes to truly bootstrap-only accounts, to avoid silently overwriting shared environments. [apolles/prisma/seed.ts:118]
...
- `pnpm prisma migrate status` (timed out after 120s against Supabase pooler URL from runtime datasource)
- `if [ -f .env ]; then ... DIRECT_URL ...; fi; if [ -f .env.local ]; then ... DIRECT_URL ...; fi` (confirmed `DIRECT_URL` is unset in both `.env` and `.env.local`)
- Updated `apolles/.env` and `apolles/.env.local` with provided `DIRECT_URL` for persistent project-level Prisma CLI connectivity.
- Updated `apolles/prisma.config.ts` to load `.env` and `.env.local` explicitly and force Prisma CLI datasource resolution to `DIRECT_URL` when present.
- `pnpm prisma migrate status` (now reaches direct host `db.wcucukfqkghnvxkifoor.supabase.co:5432`; reports migration history divergence)
- `pnpm prisma migrate deploy` (fails with P3018 because `UserRole` enum already exists while replaying `20260312000000_init_auth_ready_data_model`)
- `pnpm prisma migrate dev --name reconcile_direct_url` (fails with drift/divergence and requires `prisma migrate reset`, which would drop schema data)
- `pnpm test src/lib/prisma-schema-compat.test.ts` (failed after Prisma config update due stale assertion; fixed test expectation and re-ran pass)
- `pnpm prisma migrate reset --force` (user-approved destructive reset; migrations applied successfully and seed executed)
- `pnpm prisma migrate status && pnpm prisma migrate dev && pnpm prisma migrate deploy` (all succeeded through direct host; schema up to date)
- `pnpm test && pnpm build && pnpm typecheck` (initial run failed due missing `dotenv` type import in Prisma config; replaced with Node-based env loader)
- `pnpm test src/lib/seed.test.ts src/lib/prisma-schema-compat.test.ts && pnpm prisma migrate status && pnpm prisma migrate dev && pnpm prisma migrate deploy` (all pass)
- `pnpm test && pnpm build && pnpm typecheck` (follow-up run hit strict test typing in `seed.test.ts`; fixed and re-ran successfully)
- `pnpm test src/lib/seed.test.ts src/lib/prisma-schema-compat.test.ts && pnpm typecheck` (pass)
- `pnpm test` (full regression pass: 32 files / 196 tests)
...
- Current dev-story execution halted on Task 2 (AC #3): native Prisma Migrate remains blocked until a working direct database connection is provided via `DIRECT_URL`.
- Configured `DIRECT_URL` in project env files and verified Prisma CLI now connects via direct host; AC #3 remains blocked by migration history drift/divergence in the target Supabase database.
- Updated `src/lib/prisma-schema-compat.test.ts` assertions to match the direct-URL-first Prisma config behavior and re-validated targeted test pass.
- AC #3 is now satisfied via native Prisma Migrate commands using direct database connectivity (`migrate reset`, `migrate status`, `migrate dev`, `migrate deploy`).
- Updated seed runtime to prefer `DIRECT_URL`, tightened insecure fallback-password usage behind `ALLOW_INSECURE_SEED_DEFAULTS=1` for non-production only, and stopped role/activation overwrites during existing-user upserts.
- ✅ Resolved review finding [High]: File List reconciled to current story evidence scope.
- ✅ Resolved review finding [High]: Native Prisma Migrate execution validated on direct database path.
- ✅ Resolved review finding [Medium]: Seed runtime now prefers `DIRECT_URL`.
- ✅ Resolved review finding [Medium]: Predictable default seed passwords are now gated behind explicit local-dev opt-in.
- ✅ Resolved review finding [Medium]: Seed upserts no longer reset role/isActive for existing users.
...
- 2026-03-15: BMAD dev-story workflow — resumed Story 1.2, re-attempted Prisma migrate verification, and halted at Task 2 because `DIRECT_URL` is still not configured for native migrate execution.
- 2026-03-15: BMAD dev-story workflow — configured persistent `DIRECT_URL` in `.env` + `.env.local`, updated Prisma config to prefer direct connection for migrate commands, and re-ran migrate checks (blocked by drift requiring destructive reset/reconciliation).
- 2026-03-15: BMAD dev-story workflow — aligned Prisma config compatibility test expectations with direct URL precedence and re-ran targeted test validation.
- 2026-03-15: BMAD dev-story workflow — completed user-approved migrate reset on direct DB, validated native Prisma Migrate flow (`status`, `dev`, `deploy`), and closed open review follow-ups on seed runtime/security/upsert behavior with full test/build/typecheck passes.
